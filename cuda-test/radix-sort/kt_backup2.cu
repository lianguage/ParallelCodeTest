            ...
            int depth)
{
    ...
    //All the calculations for index for blelloch branches rely on subtraction from max index.
    //This codeblock is needed to calculate max index to subtract from.
    int maxshidx = 2*blockDim.x - 1; //@todo initialise this!!!
    if( blockIdx.x == gridDim.x - 1){
      int tidDimx = numElems%blockDim.x;
      if(tidDimx != 0){
        maxshidx = 2*tidDimx - 1;
      }
    }
    ...
    for( int d = numElems >> 1 ; d > 0  ; d >>= 1 ){
      if( tid < d){ // in the books tid = 0 is shown as the first, however in this code it refers to the last element in the respective diagram. see: http://http.developer.nvidia.com/GPUGems3/gpugems3_ch39.html
        unsigned int dest = shared[s*(2*tid + 2) - 1]; //the inclusion of blockdim and subtracting the 2*s*tid factor in the index reverses the order of if only the factor is used.
        unsigned int addn = shared[s*(2*tid + 1) - 1];
        shared[maxshidx - 2*s*tid] = dest + addn;
      }
      s *= 2;
      __syncthreads();
    }
    ...
    for(  ; s > 0 ; s >>= 1){
      if( (maxshidx - 2*s*tid - s) >= 0 && index< numElems ){
        unsigned int dest = shared[maxshidx - 2*s*tid];
        unsigned int addn = shared[maxshidx - 2*s*tid - s];
        shared[maxshidx - 2*s*tid] = dest + addn;
        shared[maxshidx - 2*s*tid - s] = dest;
      }
      __syncthreads();
    }
    */
